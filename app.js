...
//let logger = require('morgan');//日志处理被log4js代替
let cookieParser = require('cookie-parser');
let bodyParser = require('body-parser');
let path = require('path');
let fs = require('fs');
let url = require("url");
let formidable = require("formidable");//表单处理模块（包括文件上传）
let querystring = require("querystring");//请求参数序列化和反序列化模块
let node_xlsx = require('node-xlsx');//excel 存取模块
let zip_local = require('zip-local');//文件压缩模块
let ssh2 = require("ssh2");//ssh连接远程linux服务器模块
let through = require('through');
let events = require("events");
let util = require("util");
let session = require("express-session");//用来处理session
let RedisStrore = require("connect-redis")(session);//用来处理session
let async = require("async");//防止大量嵌套回调，流程处理模块
let domain = require("domain");
let hprose = require("hprose");//nodejs rpc模块（远程过程调用）
let redis = require("redis");//redis存取模块
let underscore = require("underscore");//对象、数组、函数处理工具模块
let moment = require("moment");//时间、日期，获取、计算及格式化模块
let https = require("https");//https请求
let crypto = require("crypto");//签名模块
let schedule = require("node-schedule");//定时执行模块
let Sequelize = require("sequelize");//MySQL orm模块
//加载第三方模块
let param = {
    app: app,
    fs: fs,
...
let log4js = require("./service/logConfig");
let logger = log4js.logger("production");//日志处理模块
param["logger"] = logger;
log4js.configure();
//初始化本程序运行依赖数据库
...
app.use(function (req, res, next) {
    //如果是公共路径直接放行
    let rootPath = url.parse(req.url).pathname.split("/")[1];
    ...
    let req_pathname = url.parse(req.url).pathname;
